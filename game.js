...
const createGame = (gameDiv, timerDiv, gameOverCallback, rows, columns) => {
  ...
  for (let i = 0; i < gridDiv.children.length; i++) {
    gridDiv.children[i].classList.add('card_hidden');
  }
  let index = 0;
  const showCardsAnim = setInterval(() => {
    gridDiv.children[index].classList.add('show_card_anim');
    setTimeout(() => {
      gridDiv.children[index].classList.remove('show_card_anim');
      gridDiv.children[index].classList.remove('card_hidden');
      index++;
    }, 900);
  }, 1000);
  setTimeout(() => {
    clearInterval(showCardsAnim);
  }, gridDiv.children.length * 1200);
